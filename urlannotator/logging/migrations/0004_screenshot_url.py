...
def get_screenshot_key(screenshot):
    """
    ...
    algorithm.update(screenshot)
    return algorithm.hexdigest()


def get_url(screenshot):
    base_url = 'http://' + settings.IMAGESCALE_URL
    params = {
        'width': 60,
        'height': 60,
        'url': screenshot,
        'key': get_screenshot_key(screenshot),
    }
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        import re
        r = re.compile(r'/sample/(\d+)/small')
        for log in orm.LogEntry.objects.all():
            img = log.log_val.get('sample_image', '')
            if not img:
                continue
            ...
            sample = orm['main.Sample'].objects.filter(id=sample_id)
            if not sample:
                continue

            ss = sample[0].screenshot
            url = get_url(ss)
            log.log_val['sample_image'] = url
            log.save()
    ...
    models = {
        ...
        'logging.longactionentry': {
            'Meta': {'object_name': 'LongActionEntry'},
