...
# Modified by Maciej Gol <maciej.gol@10clouds.com>:
#   - Added quality parameter '-q'.
#   - Fixed style.
#   - Fixed xvfb run command. Should follow current python path + switching to
...
def init_qtgui(display=None, style=None, qtargs=[]):
    ...
    if QApplication.instance():
        logger.debug("QApplication has already been instantiated. "
                     "Ignoring given arguments and returning existing "
                     "QApplication.")
...
if __name__ == '__main__':
    ...
    parser.add_option("-x", "--xvfb", dest="xvfb", action="store_true",
                      help="Start an 'xvfb' instance with.")
    ...
    parser.add_option("-q", dest="quality", default=-1, type="int",
                      help="Image's quality.",)
