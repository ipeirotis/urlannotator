...
# Modifed by Maciej Gol <maciej.gol@10clouds.com>:
#   - Added quality parameter to rendered images.
#   - Fixed style.

import time

from PyQt4.QtCore import (QObject, SIGNAL, Qt, QUrl, QCoreApplication, QBuffer)
from PyQt4.QtGui import (QApplication, QMainWindow, QPixmap, QImage, QColor,
    QPalette, QPainter)
...
class WebkitRenderer(QObject):
    ...
    grabWhileWindow -- If this is True a screenshot of the whole window is
                       taken. Otherwise only the current frame is rendered.
                       This is required for plugins to be visible, but it is
                       possible that another window overlays the current one
                       while the screenshot is taken. To reduce this
                       possibility, the window is activated just before it is
                       rendered if this property is set to True (default: False).
    qWebSettings -- Settings that should be assigned to the created QWebPage
                    instance.
                    See http://doc.trolltech.com/4.6/qwebsettings.html for
