...
class WizardAttributesForm(forms.Form):
    data_source = forms.ChoiceField(JOB_BASIC_DATA_SOURCE_CHOICES, label="Data source", help_text="You have 800 free URL quota provided by Odesk")
    project_type = forms.ChoiceField(JOB_TYPE_CHOICES, required=False, label="Project type")
    ...
    def clean(self):
        ...
        if Job.is_odesk_required_for_source(cleaned_data['data_source']) and not self.odesk_connect:
